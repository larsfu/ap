...
def draw_lines(x, d, black=False):
    for e in zip(x, d):
        if black==False:
            plt.plot((e[0], e[0]), (3.2, e[1]), 'r-')
        else:
            plt.plot((e[0], e[0]), (3.2, e[1]), 'k-', linewidth=2)
...
d2 = unp.uarray(d2.mean(axis=0), scipy.stats.sem(d2, axis=0))
dia = unp.uarray(dia.mean(axis=0), scipy.stats.sem(dia, axis=0))


tools.table((la, d1*1e3, d2*1e3, 1e3*(80.04e-3-d1-d2)), (r"\text{Störstelle}", r"d_1/\mm", r"d_2/mm", r"2r/mm"), 'build/diskb.tex', 'Abweichungen beim B-Scan.', 'tab:diskb', round_figures=(0,3,3,3,3))
